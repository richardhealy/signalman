...
- **App screenshots** (`docs/screenshots/`) — two Playwright-generated PNG screenshots
  added to the repository and linked in `README.md` above the Documentation table:
  `signalman-overview.png` shows the full service map, shared-library list, test-suite
  summary, and quick-start commands in a styled terminal window; `signalman-trace-anatomy.png`
  shows the annotated span tree for all three key scenarios (happy path, compensation
  unwind, reconciler divergence finding with span link). Generated by
  `scripts/screenshot.mjs` using Playwright/Chromium.
