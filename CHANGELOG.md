...
- **Integration guide** (`docs/integration.md`) — eleven-section guide covering: prerequisites (Docker, Node 20, grpcurl); the one-command `docker-compose up` setup with a port/address table for every exposed surface and step-by-step instructions for triggering the first booking and viewing its trace in Grafana Tempo; local development startup for all eight services without Docker; runnable `curl` examples for every HTTP endpoint (POST /bookings, GET /bookings/:id, GET /health) including idempotency-key usage and all response shapes; runnable `grpcurl` examples for all five internal gRPC services (Coordinator.Book, Inventory Hold/Release, Payments Authorize/Capture/Void, Supplier Confirm/Cancel, Ledger Commit/Reverse); async-event subscription on the NATS broker including a Node.js snippet for trace-context extraction; three concrete failure-injection scenarios (supplier failure → compensation trace, PSP decline, reconciler divergence); instructions for swapping in an external OTel Collector or Grafana Cloud endpoint; guidance for connecting to an external NATS JetStream cluster and an external Postgres instance; and a complete per-service environment-variable reference table. Marks documentation item (d) done.

### Documentation — 2026-06-30
- **Architecture dossier** (`docs/architecture.md`) — comprehensive nine-section dossier covering: system overview and one-liner motivation; full component map with ASCII diagrams and monorepo layout; data and control flow for the happy-path booking saga, the compensation unwind path, the async outbox→broker→inbox event pipeline (with subject routing table), and the reconciliation pass; per-library deep-dives for `@signalman/{otel,propagation,logging,interceptor,outbox,inbox,broker}`; per-service descriptions for all eight services with transport, responsibilities, gRPC surface, external boundaries, outbox events, and datastore notes; the full observability pipeline (trace propagation across gRPC, broker, and external hops; RED metrics and per-step SLO dashboards; trace-correlated structured logging); seven key design decisions with rationale (orchestrated saga, hexagonal ports, transactional outbox, idempotency at the legs, fan-out span links, settle-grace window, external calls outside the transaction); external dependencies table; and a spec-to-code traceability matrix. Marks documentation item (c) done.
