 ...
 *
 * The `payments` schema holds a single `payments` table — one row per booking.
 * The row advances through its lifecycle in place (authorized → captured or
 * voided), matching how the in-memory repository models an upsert.
 *
...
export class PostgresPaymentRepository implements PaymentRepository {
  ...
  /**
   * Create the `payments` table (and schema) if they do not exist. Safe to call
  ...
  async commit(payment: Payment, tx?: UnitOfWork): Promise<void> {
    const sql = `
      INSERT INTO "${this.schema}".payments
        (id, booking_id, amount, currency, status, authorization_id,
         capture_id, created_at, captured_at, voided_at)
