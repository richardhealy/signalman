 ...
 * commit together (the transactional-outbox guarantee).
 */
...
export interface ConfirmationRepository {
   ...
   */
  commit(confirmation: Confirmation): Promise<void>;
}
...
export class InMemoryConfirmationRepository implements ConfirmationRepository {
  ...
  async commit(confirmation: Confirmation): Promise<void> {
    this.confirmationsByBooking.set(confirmation.bookingId, { ...confirmation });
  }
}
