/**
 * Coordinates for the inventory gRPC contract, shared by the microservice (which
 * serves it) and any client that calls it.
 *
 * The package and service names mirror `proto/inventory.proto`; keeping them in
...
/** The proto `package`, as declared in `inventory.proto`. */
...
/**
 * Locate the inventory `.proto` by walking up from this module's directory for
 * the nearest `proto/inventory.proto`.
 *
...
function resolveProtoPath(): string {
  let dir = __dirname;
  for (;;) {
    const candidate = join(dir, 'proto', 'inventory.proto');
    ...
    if (parent === dir) {
      ...
      return join(__dirname, 'proto', 'inventory.proto');
    }
    dir = parent;
  }
}

/** Absolute path to the inventory `.proto`, resolved for both `src/` and `dist/`. */
export const INVENTORY_PROTO_PATH: string = resolveProtoPath();
