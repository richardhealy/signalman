 ...
 *
 * One table lives in the `ledger` schema:
 *
 * - `entries` — one row per booking's financial-record lifecycle (committed →
 *   reversed). An upsert on `booking_id` advances the row through its lifecycle
...
export class PostgresLedgerRepository implements LedgerRepository {
  ...
  /** Create the `entries` table (and schema) if they do not exist. */
